...
//>>>fork>>> Boot Screen
    ...
    //>>> Power Timer Switch
    if ((config.power.on_battery_shutdown_after_secs % 10) > 0) {                    // enabled timer switch
      title += "TS+";
    } else {
      title += "ts-";
    }
    display->setTextAlignment(TEXT_ALIGN_LEFT);
    display->drawString(x + getStringCenteredX(title), y + SCREEN_HEIGHT - 1*FONT_HEIGHT_SMALL - 1, title);
